...
**同時セッションで TS が届かない。** 3 セッションが 3 ドライバへ正しく
分散して admit されるところまでは確認できたが、いずれも 0 バイトだった。
リーダー側のログは:

```
LOOP_STATUS: total_bytes=0, consecutive_empty=540, signal=28.7dB,
             subscribers=0, state=Running
```

2 つの症状が同時に出ている:

- `subscribers=0` ―― セッションが購読していない
- `total_bytes=0` ―― ロックしているのにバックエンドから TS が来ていない

`px4rec` は同時 3 本で問題なく録画できるので、daemon 側の同時受信そのものは
動く。`bondriver/px4_daemon.rs` が複数レシーバ同時使用で TS を取れていないか、
セッション側の購読が張れていないかのどちらか (あるいは両方)。**根本原因未特定。**

単発セッションでの選局・切替・合流・退避 (§4.5) は動作しているため、
この問題は同時利用に固有。
