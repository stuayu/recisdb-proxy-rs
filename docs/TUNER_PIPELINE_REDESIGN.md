...
### 既知の制限: 同時到着した同一チャンネル要求は合流しない

同じチャンネルへの要求が**同時に**到着すると、まだ誰も開いていないため
それぞれが別のドライバに 1 本ずつ開く (上表の 3 行目: 5 セッションで 5 本)。
3 秒ずらすと 1 本に合流する (4 行目)。

`decide` は `Reserved`/`Starting` のエントリも合流先として見るので、
「誰かが開き始めた後」なら合流する。塞がっているのは
**最初のスナップショットを取ってから最初のエントリができるまでの窓**だけ。
塞ぐには「このチャンネルを開こうとしている」という意図をプールに登録する
仕組み (per-ChannelKey の single-flight) が要る。チューナーが潤沢なら
実害は「無駄に系統を使う」ことだけなので、今回は仕様として記録するに留めた。
