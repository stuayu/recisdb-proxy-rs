...
1. 同一グループの別ドライバに空きインスタンスがあれば、退避せずそこへ作成する。
2. それが無ければ、このドライバの **idle (購読者なし)** リーダー。ただし退避可否は
   `may_evict` を満たすこと。
3. idle が居なければ、**購読者のいるリーダーでも**最も優先度の低いものを
   停止する。同じく `may_evict` を満たすことが条件。
4. どれも退避できなければ**別の候補ドライバ**を試す (`decide_fallback`。
   ここも同じ規則を使う)。
5. それでも駄目なら `Reject`。
...
4. **クールダウン中の DLL が選択順位に残っていた。**
   従来は `acquire` の実行段階でクールダウンを検出してから候補を外していたため、
   選択ポリシーは毎回故障中の DLL を第一候補にし、兄弟ドライバへの再評価を
   繰り返していた。`TunerSnapshot` にクールダウン状態を含め、`decide` の新規作成・
   fallback 候補から先に除外する。同一物理チャンネルの稼働中 reader は再利用を
   優先し、全候補がクールダウン中の場合は原因を `OpenCooldown` として返す。

4. **keep-alive の残骸が新規視聴者を弾いていた** (規則の穴)。
...
`scan_space_blocking` は `BonDriverTuner::new` を直接呼ぶため、スキャンだけが
スロット permit を取らず、`max_instances` の会計にも `active_tuners` にも
現れなかった。別ドライバを走査する限り問題ないが、単一 open のデバイスで
視聴者が使っているドライバを走査しようとすると衝突する。

`TunerPool::begin_scan(dll_path, max_instances)` を追加し、スキャンも
**視聴・録画と同じスロット予約を通す**ようにした。

- `ScanReservation` が `SlotPermit` を保持する。スキャンが走っている間その
  ドライバは満杯として扱われ、視聴要求は別ドライバへ回る。
- 予約が取れなければスキャンは**譲る**。スキャンは最も優先度の低い作業なので
  (DESIGN.md §4.4)、退避はせず次のスケジューラ tick を待つ。
  `next_scan_at` は据え置くので要求は失われない。
- 走査中のドライバは `scanning_drivers` として記録され、失敗・タイムアウトを
  含めて `Drop` で必ず解放される。

**Web UI**: `/api/stats` に `scanning_tuners` / `scanning_drivers`、
`/api/bondrivers` に `is_scanning` を追加。概要の「アクティブチューナー」は
`3 (+スキャン 1)` の形で内訳を出し、BonDriver 一覧には「状態」列で
「スキャン中」を表示する。**チューナーが埋まっている理由が利用者から見えないと、
視聴できない原因を追えない**ため。

実機確認: 視聴者が使っているドライバへスキャンを要求すると 3 回連続で
「deferring its scan」となり視聴は無傷、視聴終了後 (keep-alive 満了後) に
スキャンが開始し `scanning_drivers` に現れた。

### 付随して見つけた実バグ: 異常切断でチューナーが解放されない

上の検証中に、クライアントが黙って接続を切ると (`Connection reset by peer`)
セッションのリーダーが解放されないことが分かった。`run()` のループは
ソケットエラーを `?` で伝播するため、**末尾の `cleanup()` に到達せず**、
keep-alive close が予約されないままリーダーが走り続ける。

購読者ゼロかつ idle-close 予約なしのエントリは設計上「セットアップ中」と
みなして回収対象外にしているので、この状態のリーダーは**誰にも回収されない**。
P1b でスロットが permit 制になったため、症状は「そのドライバがプロセスの寿命の
間ずっと埋まったまま」になる。リファクタ以前からある構造だが、permit 制に
した今は実害が大きい。

`run()` を「ループ → 結果に関わらず `cleanup()` → 結果を返す」形に変更した。
実機で、クライアントを落とした後 keep-alive 満了でチューナーが解放されることを
確認 (`active_tuners` 1 → 0)。
