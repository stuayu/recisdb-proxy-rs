...
接続情報は`recisdb://pair?endpoint=...&code=...`形式でコピーできます。QR生成は
未実装ですが、外部CDNなしで動作する文字列フローを維持しています。

responsive確認スクリプトはPlaywrightが利用可能ならfile URL上で実測し、ブラウザが
未導入の場合はビルド済みCSS・構成図ソースの静的検査へフォールバックします。
