...
INFOレベルは「運用者が流し読みして状況が分かる」量に保つ。ストリーミング中に
毎秒・毎チャンク発生する事象をINFOに置くと、1日で数十万行に達してログが
使い物にならなくなる（2026-08-10の本番ログは102万行/214MBで、うち99.4%が
下記2種類のログだった）。

判断基準:

- **状態が変わったとき**（選局成功、リーダー起動/停止、接続/切断、退避）はINFO。
- **同じ状態が続いている間の観測値**（送信バイト数、空振り、ポーリング応答）は
  DEBUG、またはしきい値を超えたときだけINFO。
- 異常が**続いている**ことを伝えたい場合は、発生ごとではなく一定間隔で
  「連続N回、直近M件は抑制」の形にまとめる。

現在この方針で間引いているもの:

| 事象 | 扱い | 実装 |
| --- | --- | --- |
| `wait_ts_stream` の空振り / `get_ts_stream` の WouldBlock | 連続50回（≒5秒間無データ）ごとに1行 | `tuner/shared.rs` の `should_log_empty_streak` |
| セッションのTS送信状況 | DEBUG（同じ数字はダッシュボードのクライアント一覧に出る） | `server/session.rs` |
| ダッシュボードのポーリング（`/api/stats`, `/api/clients` など）のアクセスログ | 成功時はDEBUG、失敗時はINFO | `web/mod.rs` の `is_dashboard_poll` |
| BonDriverのオープン連続失敗 | 1ドライバにつき60秒に1回のWARNに集約し、抑制件数を併記 | `tuner/open_backoff.rs`（詳細は `docs/TUNER_PIPELINE_REDESIGN.md`） |

`should_log_empty_streak` は「連続回数がしきい値に達したときだけ出す」という
条件になっている。連続回数はデータが届くたびに1へ戻るため、`% 50 == 1` の
ような剰余での間引きは**通常のストリーミング中に毎回成立してしまう**
（この誤りが上記68万行の原因だった）。同種の間引きを足すときは同じ罠に注意する。
