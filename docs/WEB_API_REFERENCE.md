...
ノードトランスポートは常時起動する（未ペアリング時は待機のみ）。これらは
**ダッシュボード側**のAPIで、通常の `/api/*` 認証に従う。
待ち受け先はプロキシ本体の `[server] listen` と同じIP・ポート+1で自動決定し、
ペアリングコード発行レスポンスの `node_listen_addr` に表示する。
...
| POST | `/api/nodes/local` | ローカルノードの表示名をDBへ保存し、再起動なしでピアへの表示へ反映 |
| POST | `/api/nodes` | リモートノードの手動登録・編集（エンドポイントは丸ごと置換） |
| POST | `/api/nodes/:id/state` | リモートノードの利用を一時停止/再開（資格情報と経路キャッシュは保持） |
| DELETE | `/api/nodes/:id` | リモートノードと資格情報・経路キャッシュを削除 |
