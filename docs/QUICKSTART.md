...
起動すると、まず行いたい作業を3つから選びます。用途ごとに独立しているので、必要な作業だけを実行できます。

| 種類 | 内容 |
|---|---|
| **本体をインストール (全自動)** | チューナーの検出・登録、px4_drv ドライバの自動インストール、ブラウザ視聴 (プレビュー) の準備、OSサービス登録までまとめて行います。はじめて設定する場合はこちら。ドライバ未導入のチューナーが複数あっても、1台ずつ順に自動で処理します。 |
| **本体をインストール (ドライバ導入をスキップ)** | ドライバの自動インストールを行いません (未導入なら画面に案内が出るだけ)。既にドライバを導入済みの環境や、登録するチューナーを自分で選びたい場合向け。ダウンロードを伴うプレビュー準備は既定でOFFになっており、確認画面で個別に選べます。 |
| **クライアントDLLを差し替える** | TVTest/EDCB 側に配置済みの `BonDriver_NetworkProxy*.dll` を新しい版に差し替えるだけのモード。更新元DLLと更新先フォルダを指定して実行します。サーバー本体・設定ファイル・データベースには一切触れません。 |

DLL差し替えモードの「更新元のDLL」は、未指定なら次の順に自動で探します。

1. インストール先の `client-config\BonDriver_NetworkProxy.dll` (セットアップ実行後に生成されるもの)
2. `recisdb-proxy-setup.exe` と同じフォルダにある `BonDriver_NetworkProxy.dll` (リリースzipを展開しただけの状態)

更新先フォルダ以下 (サブフォルダ含む) の `BonDriver_NetworkProxy` で始まる `.dll` がすべて対象になり、
別名で複製したファイル名はそのまま保たれます。

なお本体インストールの2モードでも、インストール先の画面で「クライアントDLLの一括更新」先を指定しておけば、
完了画面から同じ一括更新を実行できます。

### 画面の見やすさについて

セットアップ画面はユニバーサルデザインフォント (BIZ UDゴシック / UDデジタル教科書体、無ければ
Noto Sans CJK や游ゴシック) を優先して使い、本文17px・見出し27px・ボタン高さ46pxで表示します。
配色は明るい背景に高コントラストの文字で固定しており、OS側がダークテーマでも変わりません。
