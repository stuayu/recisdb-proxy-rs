...
- **チューナーのファームウェアが必要な機種がある。** PX-S1UD / PX-Q1UD などの
  Siano チップ機は `/lib/firmware/isdbt_rio.inp` を要求するが、これを同梱して
  いるディストリはほとんどない。ファームウェアが無い場合でも
  `/dev/dvb/adapterN` は作られ、本バックエンドの ioctl もすべて成功するため、
  「エラーは出ないのにどのチャンネルもロックしない」という分かりにくい形で
  失敗する。`dmesg | grep -i firmware` で確認できる。
  PLEX 配布のドライバアーカイブに含まれる `isdbt_rio.inp` を
  `/lib/firmware/` へ置いて再起動すれば解消する(ファームウェア自体は
  アーキテクチャ非依存なので arm64 でも x86 版アーカイブのものが使える)。
