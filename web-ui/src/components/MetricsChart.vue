...
function pidValue(item: unknown, key: string, position: number): unknown {
  ...
  if (item && typeof item === 'object') {
    ...
    return record[key] ?? (key === 'errors' ? record.cc_errors : key === 'packets' ? record.total : undefined)
  }
  return undefined
}
  ...
  <section ref="container" class="metrics-panel">
    ...
    <section class="quality-panel">
      ...
      <div class="stat-grid compact-stats">
        <article class="stat-card"><span>broadcast lag（chunks）</span><strong v-text="String(quality.loss_broadcast_lag_chunks ?? 0)"></strong></article>
        <article class="stat-card"><span>TS queue drop（chunks）</span><strong v-text="String(quality.loss_ts_queue_chunks ?? 0)"></strong></article>
        <article class="stat-card"><span>encoder stall（events）</span><strong v-text="String(quality.loss_encoder_stall_events ?? 0)"></strong></article>
      </div>
      <h4>ロス上位 PID（CC error）</h4>
      <div class="table-region">
        <table v-if="topLossPids.length" class="data-table compact"><thead><tr><th>PID</th><th>パケット数</th><th>CCエラー数</th></tr></thead><tbody>
          <tr v-for="(pid, index) in topLossPids" :key="index"><td data-label="PID" v-text="String(pidValue(pid, 'pid', 0) ?? '—')"></td><td data-label="パケット数" v-text="String(pidValue(pid, 'packets', 1) ?? '—')"></td><td data-label="CCエラー数" v-text="String(pidValue(pid, 'errors', 2) ?? '—')"></td></tr>
        </tbody></table>
