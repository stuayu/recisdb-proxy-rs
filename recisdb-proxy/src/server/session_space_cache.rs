...
pub(super) fn clear_caches(channel_map_cache: &mut ChannelMapCache, space_list_cache: &mut SpaceListCache) {
...
pub(super) async fn ensure_channel_map_with_region(
    ...
    let all = {
        ...
        match db.get_all_channels_with_drivers() {
            Ok(v) => v,
            Err(e) => {
                debug!("[Session {}] ensure_channel_map_with_region: failed to get channels: {}", session_id, e);
                Vec::new()
            }
        }
    };
    ...
    let map = if !group_driver_paths.is_empty() {
        client_view::build_channel_list(&all, |path| group_driver_paths.iter().any(|p| p == path), region_name)
    } else {
...
pub(super) async fn ensure_space_list(
    ...
    let (cache_key, is_group) = if !group_driver_paths.is_empty() {
        (format!("group_{}", current_group_name.unwrap_or("unknown")), true)
    } else {
        if current_tuner_path.is_empty() {
            debug!("[Session {}] ensure_space_list: tuner_path is empty", session_id);
            return Vec::new();
        }
    ...
    if let Some(v) = space_list_cache.get(&cache_key) {
        trace!("[Session {}] ensure_space_list: using cache for {} (spaces: {:?})", session_id, cache_key, v);
    ...
    let all = {
        ...
        match db.get_all_channels_with_drivers() {
            Ok(v) => v,
            Err(e) => {
                debug!("[Session {}] ensure_space_list: failed to get channels: {}", session_id, e);
                Vec::new()
            }
        }
    };
    ...
    debug!("[Session {}] ensure_space_list: final spaces for {}: {:?}", session_id, cache_key, list);
...
pub(super) fn map_space_idx_to_actual_with_region(
    ...
    let list = get_space_list_with_names(group_driver_paths, current_group_name, current_tuner_path, space_list_cache);
...
pub(super) fn get_space_list_with_names(
    ...
    if !group_driver_paths.is_empty() {
        ...
        return space_list_cache.get(&cache_key).cloned().unwrap_or_default();
    }
    if current_tuner_path.is_empty() {
        return Vec::new();
    }
    space_list_cache.get(current_tuner_path).cloned().unwrap_or_default()
}
