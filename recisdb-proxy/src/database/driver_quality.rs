...
impl Database {
    ...
    pub fn get_driver_quality_stats(
        &self,
        bon_driver_id: i64,
    ) -> Result<Option<DriverQualityStats>> {
    ...
    pub fn record_driver_runtime_sample(
        &self,
        bon_driver_id: i64,
        ...
        let latency_score = |value: Option<f64>, soft_ms: f64, hard_ms: f64| -> f64 {
            ...
            if value <= soft_ms {
                1.0
            } else if value >= hard_ms {
                0.20
            } else {
                1.0 - ((value - soft_ms) / (hard_ms - soft_ms)) * 0.80
            }
        };
...
mod runtime_tests {
    ...
    fn slow_but_successful_driver_is_demoted() {
        ...
        let id = db
            .insert_bon_driver(&NewBonDriver::new("slow.dll"))
            .unwrap();
        for _ in 0..5 {
            db.record_driver_runtime_sample(
                id,
                DriverRuntimeSample {
                    open_ms: Some(2_000),
                    tune_ms: Some(8_000),
        ...
        assert!(
            score < 0.6,
            "slow-but-successful driver must not remain perfect: {score}"
        );
    }
    ...
    fn a_fast_driver_keeps_a_perfect_score_and_outranks_a_slow_one() {
        let db = Database::open_in_memory().unwrap();
        let fast = db
            .insert_bon_driver(&NewBonDriver::new("fast.dll"))
            .unwrap();
        let slow = db
            .insert_bon_driver(&NewBonDriver::new("slow.dll"))
            .unwrap();
        ...
        assert_eq!(
            fast_score, 1.0,
            "a comfortably fast driver must not be penalised"
        );
        assert!(
    ...
    fn a_driver_that_never_produces_ts_is_demoted_hard() {
        let db = Database::open_in_memory().unwrap();
        let id = db
            .insert_bon_driver(&NewBonDriver::new("silent.dll"))
            .unwrap();
        for _ in 0..3 {
            db.record_driver_runtime_sample(
                id,
        ...
        assert!(
            score < 0.5,
            "a driver that never delivers TS must be demoted: {score}"
        );
    }
    ...
    fn stalls_demote_a_driver_that_is_otherwise_fast() {
        let db = Database::open_in_memory().unwrap();
        let id = db
            .insert_bon_driver(&NewBonDriver::new("hiccup.dll"))
            .unwrap();
        for _ in 0..4 {
            db.record_driver_runtime_sample(
                id,
        ...
        assert!(
            score < 1.0,
            "a stalling driver must not score perfectly: {score}"
        );
    }
    ...
    fn open_failures_demote_a_driver_even_after_its_cooldown_expires() {
        let db = Database::open_in_memory().unwrap();
        let id = db
            .insert_bon_driver(&NewBonDriver::new("flaky.dll"))
            .unwrap();
        for _ in 0..4 {
            db.record_driver_runtime_sample(
                id,
        ...
        assert!(
            score < 0.7,
            "repeated open failures must persist as low health: {score}"
        );
    }
    ...
    fn a_driver_with_no_samples_scores_neutral() {
        ...
        assert_eq!(
            db.get_driver_quality_score_by_path("absent.dll").unwrap(),
            1.0
        );
    }
}
